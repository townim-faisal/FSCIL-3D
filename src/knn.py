...
class KMeans:
  ...
  Attributes:
    centroids: torch.Tensor, shape: [n_clusters, n_features]
      cluster centroids
  '''
  def __init__(self, n_clusters, max_iter=100, tol=0.0001, verbose=0, mode="euclidean", minibatch=None, centroids=None):
    if mode!='euclidean' and mode!="cosine":
      raise Exception("Illegal argument: mode must be 'euclidean' or 'cosine' in KMeans")
  ...
  def max_sim(self, a, b):
    ...
    if device == 'cpu':
      ...
      for i in range(ratio):
        ...
        del sub_sim
        msv.append(sub_max_sim_v) #dist
        msi.append(sub_max_sim_i) #index
      if ratio == 1:
  ...
  def cluster_sim(self, a, b=None):
    """
      Compute similarity (or minimum distance) of each vector
      in a with all of the vectors in b
      Parameters:
      a: torch.Tensor, shape: [m, n_features]
      b: torch.Tensor, shape: [n, n_features]
